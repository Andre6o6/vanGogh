...
$(document).ready(function () {
	...
	if (JSON.parse(localStorage.getItem('expires_at')) < new Date().getTime()) {
		document.getElementById('button-export').style.display = 'none';
		document.getElementById('button-login').style.display = 'inline-block';		
	} else {
		document.getElementById('button-export').style.display = 'inline-block';
	...
	Export = function (imgname) {
		var imagename = imgname;
		var source = app.stage.toDataURL();
		...
		$.ajax({
		  type: "POST",
		  url: "https://us-central1-vangogh-editor.cloudfunctions.net/createImage",
		  data: json,
		}).always(function( msg ) {
			console.log( msg );
		});	
	...
	$('#button-export').click(function () {
		...
		$('#dialog-export input').val('').attr('disabled', false).focus();		
	});
	$('#dialog-export button.button-ok').click(function () {
		$('#dialog-export input').attr('disabled', true);
		Export($('#dialog-export input').val());
		hideDialog('#dialog-export');
	});
